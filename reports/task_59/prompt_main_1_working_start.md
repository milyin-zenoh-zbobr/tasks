...
- planning
  - ✅ Plan approved and checklist created. Approach: add `Transport::new_from_fields() [ctx_rec_4]
    - [ ] Add `Transport::new_from_fields()` constructor gated with `#[zenoh_macros::inter [ctx_rec_2]
    - [ ] Verify: `cargo build` with `internal` feature, with `internal` + `shared-memory` [ctx_rec_3]
