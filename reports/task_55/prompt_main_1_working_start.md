...
- planning
  - ✅ Plan approved and checklist items created for prompts-should-be-hash implementat [ctx_rec_10]
    - [ ] Update config structs to use IndexMap<String, TomlOption<PathBuf>> for prompts [ctx_rec_5]
    - [ ] Update resolve_paths for all three structs to iterate map values [ctx_rec_6]
    - [ ] Rewrite prompt_files_for_stage() to use three-level map merge [ctx_rec_7]
    - [ ] Update default config in init.rs to use named prompt slots [ctx_rec_8]
    - [ ] Update tests for new prompts map structure [ctx_rec_9]
