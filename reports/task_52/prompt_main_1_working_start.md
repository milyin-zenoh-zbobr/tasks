...
- planning
  - ✅ Plan approved and checklist created: (1) Add `task select` subcommand replacing [ctx_rec_4]
    - [ ] Add `task select` subcommand and remove `--select` from `task list` [ctx_rec_2]
    - [ ] Make `--logs` accept an optional boolean parameter with last-value-wins semantic [ctx_rec_3]
