...
# Current task: update commands

# Task description

implement https://github.com/milyin/zbobr/issues/318

# Destination branch: main

# Work branch: zbobr_fix-52-update-commands

# Context

- planning
  - 💬 Plan for issue #318: (1) Replace `task list --select` with dedicated `task selec [ctx_rec_1]
- user milyin: proceed with plan
- planning
  - ✅ Plan approved and checklist created: (1) Add `task select` subcommand replacing [ctx_rec_4]
    - [x] Add `task select` subcommand and remove `--select` from `task list` [ctx_rec_2]
    - [x] Make `--logs` accept an optional boolean parameter with last-value-wins semantic [ctx_rec_3]
- working
  - ✅ Implemented both changes from issue #318: (1) replaced `task list --select` with [ctx_rec_5]
- reviewing
  - ❌ Found a correctness regression in the new `--logs` parsing: supported forms are [ctx_rec_6]
