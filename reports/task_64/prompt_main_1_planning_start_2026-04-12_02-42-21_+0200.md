...
- Use MCP `report_intermediate` to present the plan for user review when the plan is not yet approved
- Use MCP `report_success` to send the approved plan to implementation
...
7. **Check for user approval**:
   ...
   - If approval is confirmed (in the last comment or task description):
     - Call `report_success` with a brief rationale (why this approach was chosen, key design decisions, important constraints, chosen analog).
   - If approval is NOT confirmed (including any doubt):
     - Present the plan by calling `report_intermediate` with a description of the proposed approach and rationale.
