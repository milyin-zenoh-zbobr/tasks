...
- planning
  - ✅ Plan approved and checklist ready: add `auto_sync_fork: bool` config (default tr [ctx_rec_4]
    - [ ] Add `auto_sync_fork: bool` config field (default true) to ZbobrRepoBackendGithub [ctx_rec_2]
    - [ ] Add `maybe_sync_fork` method and call it in `ensure_bare_clone_github` before `g [ctx_rec_3]
