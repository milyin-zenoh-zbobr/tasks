...
4. Finish by calling one of:
   - `report_success` — the plan is solid and ready for implementation. You may include minor suggestions or observations in the message, but they must not block progress.
...
- planning
  - ✅ Revised implementation plan for connectivity API (Transport, Link, events listen [ctx_rec_7]
- plan_review_adversarial
  - ❌ The plan is not implementation-ready: it misses required Kotlin surface/support [ctx_rec_8]
- planning
  - ✅ Revised connectivity API implementation plan addressing all blocking issues from [ctx_rec_9]
