...
- planning
  - ✅ Plan approved and checklist items created for implementing connectivity API 2 [ctx_rec_10]
    - [ ] Update zenoh-c submodule to milyin-zenoh-zbobr/zenoh-c branch zbobr_fix-60-trans [ctx_rec_3]
    - [ ] Copy base connectivity API files from upstream connectivity_api branch [ctx_rec_4]
    - [ ] Redesign Transport as pure Go struct (no C ownership, no Drop/Clone) [ctx_rec_5]
    - [ ] Redesign Link as pure Go struct (no C ownership, no Drop/Clone) [ctx_rec_6]
    - [ ] Change Transport filter in options from *Transport to option.Option[Transport] [ctx_rec_7]
    - [ ] Implement transport filtering using zc_internal_create_transport with move seman [ctx_rec_8]
    - [ ] Update examples and tests: remove Drop() calls, use option.Some for transport fi [ctx_rec_9]
