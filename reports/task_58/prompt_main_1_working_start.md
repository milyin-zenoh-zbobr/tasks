...
- planning
  - ✅ Completed thorough exploration of the zbobr Rust config system, covering the con [ctx_rec_2]
    - [ ] Create `TomlOption<T>` enum in `zbobr-utility` [ctx_rec_3]
    - [ ] Update `config_struct` proc macro to use `TomlOption<T>` [ctx_rec_4]
    - [ ] Update manually-defined config types to use `TomlOption<T>` [ctx_rec_5]
    - [ ] Fix consumer code that reads from Toml types [ctx_rec_6]
    - [ ] Add tests for `TomlOption<T>` and NaN config clearing [ctx_rec_7]
